...
- ✅ **hash 隔离**：按文件内容 SHA-1 区分不同翻译对象——不同游戏的
  ...
  任务启动时**快照工程身份**，中途换文件会丢弃结果而不是写错地方；
  切换文件后界面缓存自动作废。有任务在跑时不允许切换文件
- ✅ **术语库自动去伪**：用**全文**校验提取结果 —— 剔除整句（模型爱把
  「〜を表示しないようにします。」塞进口癖）、句型占位、全文未出现的幻觉条目，
  以及只作为更长词条片段出现的碎片（已有「バイドーラ」时单收的「バイド」会被剔除）
- ✅ **译法分歧可点选**：同一原文在不同批次被译成多种写法时，界面直接列出候选按钮
  （带被提出的次数），点一下即采用并保存
- ✅ **单条重译够快**：逐条重译走精简提示词（角色表按本行相关性裁剪），
  实测提示词小 57%
- ✅ **多选批量重译**：修正页可勾选多条一次重译（含「选中本页未翻译」一键选），
  整批没匹配上的会自动逐条兜底再试
- ✅ **多行原文可靠译出**：键比对把所有空白（含换行）视为等价，
  并对归一化歧义做保护；收敛轮批次逐轮缩小到单条，顽固多行条目不再永久漏译
...
- ✅ **不会留下后台僵尸进程**：界面上有「退出程序」按钮可立即关闭；关掉浏览器后
  默认 10 分钟自动退出（有任务在跑时不退），时长可在「设置 → 程序行为」调整
...
| **设置** | API（base_url/key/model/温度/max_tokens/超时/思考开关+连接测试）、流水线参数（并发/批大小/输出目录/上下文行数）、**分析采样量（比例 + 上下限，带实际条数预览）**、翻译风格（R18/名词风格/精修/自定义提示词）、**程序行为（无人连接时自动退出）** |

### 关闭程序

打包版是窗口模式（刻意不弹黑色控制台），所以关掉浏览器后进程仍会在后台。三种关闭方式：

| 方式 | 说明 |
|------|------|
| 界面右上角 **「退出程序」** | 立即关闭，推荐 |
| 什么都不做 | 关掉浏览器 **10 分钟**后自动退出 |
| 关掉最小化的黑窗口 | 仅用命令行 / `启动TransIt.bat` 启动时有 |

**有任务在跑时不会自动退出**。自动退出时长可在「设置 → 程序行为」调整，填 `0` 表示不自动退出。
命令行下可用 `--exit-when-idle <秒>` 覆盖。

> 自动退出以「浏览器的最后一次请求」为心跳（页面每 1.5 秒轮询一次），
> 因此关掉标签页后心跳停止即可判定为无人使用。页面若失去连接会明确提示
> 「程序可能已退出，请重新启动」，而不是抛一串看不懂的报错。
