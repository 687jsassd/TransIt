...
2. **分析**：按比例采样待译文本 → 让 LLM 产出**风格圣经**：世界观与基调、命名方针、
   各类文本的语气指南、**角色表**（性别/语气/自称/称呼关系），并分块提取专有名词建术语库
3. **精翻**：带世界观 + 角色表 + 术语库约束的批量翻译（OpenAI 兼容 API，任意自定义端点），
   支持并发、断点续传、占位符保留
4. **校对**：逐句检查/修改译文，可对单条要求 AI 重译
5. **导出**：输出与输入完全同格式的完整 JSON（未翻译条目原样保留，可继续补齐）

### 分析的三阶段

| 阶段 | 产出 | 为什么需要 |
|------|------|-----------|
| 1 风格圣经 | 世界观、游戏类型/舞台/剧情/玩家视角/基调、**命名方针**、**语气指南**（旁白·对白·菜单·成人场景）、翻译注意 | 让每一次翻译请求都带着统一的文风与命名策略 |
| 2 角色表 | 每个角色的性别、身份、性格、说话风格、第一人称自称、被称呼方式 | 日语不标性别而中文必须选「他/她」；自称与称呼方式直接决定对白语气 |
| 3 术语提取 | 分类术语库（角色名/物品/势力/地名/专业术语/口癖…） | 保证全篇译名一致；角色表会喂给每一块，避免同一角色被译成多种写法 |

采样按**待译条数的比例**（默认 10%）并受上下限约束（默认 200~1000 条），且采用
**窗口采样**：把全文切成若干段、每段取一段连续文本。样本保持原文顺序并覆盖剧本的
开头/中段/结尾 —— 世界观分析因此能看到连贯对白，而不是零散单句。

```jsonc
// config.json
"pipeline": {
  "sample_ratio": 0.10,   // 采样比例（占待译条数）
  "sample_min": 200,      // 下限：短文件也至少采这么多
  "sample_max": 1000      // 上限：长文件最多采这么多，控制分析成本
}
```

实际条数 = `待译条数 × sample_ratio`，再夹到 `[sample_min, sample_max]`，
且不会超过文件本身；待译条数不足下限时取全量。旧版的固定条数键 `sample_size`
会被忽略（并在启动时提示），不再有"改了没生效"的困惑。
...
- ✅ **风格圣经可视化可编辑**：世界观、命名方针、语气指南、**角色表**（性别是下拉框）
  都能在界面里直接改，改完立即影响后续翻译
- ✅ **译法分歧可裁决**：不同术语块给出不同译法时，保留先出现的并列出被忽略的候选
- ✅ **单条重译**：对某一句要求 AI 重译，可附「本次修改要求」（如"改口语化一点"），
  不必为改一句重跑上千条
- ✅ **连续校对**：`Ctrl+Enter` 保存并下一条、`Alt+Enter` 重译，不用反复开合弹窗
- ✅ **修正页筛选**：排除「原样保留」条目、只看未翻译、按分类过滤、搜索原文**或译文**
...
| **术语库** | **风格圣经可视化**：世界观/命名方针/语气指南可编辑；**角色表**（性别下拉框，译名/语气/自称/称呼均可改）；**译法分歧**列表；词条增删改（双击单元格）、搜索过滤、自动合法性过滤 |
| **译文修正** | 全部条目浏览/搜索（原文**或译文**）、排除「原样保留」、只看未翻译、分类与全局统计、双击逐句修正（即时写回进度文件）、**单条 AI 重译**（可附修改要求）、`Ctrl+Enter` 保存并下一条、分页 |
| **设置** | API（base_url/key/model/温度/max_tokens/超时/思考开关+连接测试）、流水线参数（并发/批大小/**采样比例与上下限**）、翻译风格（R18/名词风格/精修/自定义提示词） |
...
> tkinter 界面是开发期的备选入口，功能少于 WebUI：没有角色表编辑、单条重译、
> 条目分类筛选与统计。发布包中不包含它（以 WebUI 为唯一图形界面）。
